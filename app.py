...
try:
    ...
    print("yfinance nicht installiert – Dummy-Daten werden verwendet.")
...
def load_stock_data(ticker: str, period: str = "1y") -> pd.DataFrame:
    if YFINANCE_AVAILABLE:
        ...
        df = _generate_dummy_data(ticker, days=252)
...
def _generate_dummy_data(ticker: str, days: int = 252) -> pd.DataFrame:
    rng = np.random.default_rng(seed=sum(ord(c) for c in ticker))
    dates = pd.bdate_range(end=pd.Timestamp.today(), periods=days)
    price = 150.0
    prices = []
    for i in range(days):
        shock = 5.0 if i in [40, 90, 150, 200] else 1.0
        price *= np.exp(rng.normal(0.0003, 0.015) * shock)
        prices.append(round(price, 2))
    closes = np.array(prices)
    return pd.DataFrame({
        "Date":   dates,
        "Open":   np.round(closes * (1 + rng.normal(0, 0.003, days)), 2),
        "High":   np.round(closes * (1 + np.abs(rng.normal(0, 0.008, days))), 2),
        "Low":    np.round(closes * (1 - np.abs(rng.normal(0, 0.008, days))), 2),
        "Close":  closes,
        "Volume": rng.integers(10_000_000, 80_000_000, days).astype(float),
    })
...
if __name__ == "__main__":
    ...
    print("  Starte jetzt: /opt/anaconda3/bin/python dashboard_server.py")